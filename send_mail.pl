...
sub query_to_xlsx {
    ...
    if ( $main::decoded_json->{"Author"} ne "" ) {
        $workbook->set_properties( author => $main::decoded_json->{"Author"} );
    }
    else {
        $workbook->set_properties( author => $send_mail_defaults::AUTHOR );
    }
    ...
    do {
        ...
        if ( scalar @cols > 0 ) {
            ...
            if ( $main::decoded_json->{ "ws" . $count }{'name'} ne "" ) {
                $sheet = $workbook->add_worksheet( $main::decoded_json->{ "ws" . $count }{'name'} );
            }
            else {
            ...
            if ( $main::decoded_json->{ "ws" . $count }{'keep_leading_zeros'} eq "1" ) {
                $sheet->keep_leading_zeros();
            }
            my $use_string_writer = 1;
            if ( $main::decoded_json->{ "ws" . $count }{'use_string_writer'} eq "0" ) {
                $use_string_writer = 0;
            }
            ...
            if ( not( $main::decoded_json->{ "ws" . $count }{'noheader'} eq "1" ) ) {
                foreach (@cols) {
                    ...
                    if ( $_ ne "" ) {
                        if ( $hun_corr eq 1 ) {
                            if ( $use_string_writer == 1 ) {
                                $sheet->write_string( 0, $cnum, fix_chars($_), $bold_format );
                            }
                            else { $sheet->write( 0, $cnum, fix_chars($_), $bold_format ); }
                        }
                        else {
                            if ( $use_string_writer == 1 ) { $sheet->write_string( 0, $cnum, $_, $bold_format ); }
                            else {
                                $sheet->write( 0, $cnum, $_, $bold_format );
                            }

                        }
                        if ( $autofit == 1 ) { store_string_widths( $sheet, 0, $cnum, $_ ); }
                    }
                    $cnum++;
                }
                $i = 1;
                ...
                while ( @row = $sth->fetchrow_array ) {
                    if ( $hun_corr eq 1 ) {
                        ...
                        for my $cell (@row) {
                            if ( $cell ne "" ) {
                                if ( $use_string_writer == 1 ) {
                                    $sheet->write_string( $i, $collnum, fix_chars($cell), $default_format );
                                }
                                else { $sheet->write( $i, $collnum, fix_chars($cell), $default_format ) }
                                if ( $autofit == 1 ) { store_string_widths( $sheet, $i, $collnum, $cell ) }
                            }
                            $collnum++;
                        }
                    }
                        ...
                        for my $cell (@row) {
                            if ( $cell ne "" ) {
                                if ( $use_string_writer == 1 ) {
                                    $sheet->write_string( $i, $collnum, $cell, $default_format );
                                }
                                else { $sheet->write( $i, $collnum, $cell, $default_format ); }
                                if ( $autofit == 1 ) { store_string_widths( $sheet, $i, $collnum, $cell ) }
                            }
                            $collnum++;
                        }
            ...
            foreach (@cols) {
                if ( $main::decoded_json->{ "ws" . $count }{ "col_len" . $cnum } ne "" ) {
                    $sheet->set_column( $cnum, $cnum, $main::decoded_json->{ "ws" . $count }{ "col_len" . $cnum } );
                }
                $cnum++;
            }
            my $rcnt2 = $rcnt + 1;
...
sub select_to_mail {
    try {
        ...
        my_sendmail( $$main::line{"msg_from"}, $$main::line{"on_error"}, "ERROR: " . $$main::line{"msg_subject"}, $_ );
    };
}
...
sub select_to_file {
    try {
        ...
        my_sendmail(
            $$main::line{"msg_from"},
            $$main::line{"msg_to"},
            $$main::line{"msg_subject"},
            $$main::line{"msg_body"}
        );
    }
    catch {
        my_sendmail( $$main::line{"msg_from"}, $$main::line{"msg_to"}, "ERROR: " . $$main::line{"msg_subject"}, $_ );
    };
}
...
sub select_to_file_and_send {
    try {
        ...
        send_xls_to_mail(
            ...
            $$main::line{"msg_subject"},
            $$main::line{"msg_body"},
            "$send_mail_defaults::basedir" . $$main::line{"file_name"}
        );
    }
    catch {
        my_sendmail( $$main::line{"msg_from"}, $$main::line{"on_error"}, "ERROR: " . $$main::line{"msg_subject"}, $_ );
...
$main::dbh =
  DBI->connect( "DBI:mysql:database=$send_mail_defaults::SQL_DB;host=$send_mail_defaults::SQL_HOST;"
...
while ( $main::line = $sth->fetchrow_hashref ) {
    try {
        ...
        $main::dbh->do( "update $send_mail_defaults::SQL_TABLE set msg_ready=1 where rowid=" . $$main::line{"rowid"} );
    }
    catch { print "Error: " . $_ . "\n"; };
